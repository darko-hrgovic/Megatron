(function () {
  ...
  Drupal.behaviors.megatronDrawer = {
    attach: function (context, settings) {
      // In spite of Megatron loading this once in drupal_add_js, Advanced Forum causes this to load twice
      // Could be solved with .once() but we're in vanilla JS land
      // Only fire if our drawer-once div doesn't exist
      var drawerOnce = document.getElementsByClassName("drawer-once");
      if (drawerOnce.length === 0) {
        // Create a DOM element to check on next load
        var drawerOnce = document.createElement("div");
        drawerOnce.className = "drawer-once";
        document.body.appendChild(drawerOnce);
